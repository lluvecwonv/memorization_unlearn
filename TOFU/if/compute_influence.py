...
class DifferentialTokenScorer(Analyzer):
        ...
        with self.profiler.profile("Load All Factors"):
            if factors_name is None:
                ...
                try:
                    ...
                    print(f"✅ Loaded {len(loaded_factors)} factor groups" if loaded_factors else "⚠️ No factors loaded")
                    loaded_factors = loaded_factors or {}
    ...
    def _differential_token_computation(self, loaded_factors, forget_loader, retain_loader, forget_sample_loader, tracked_module_names):
        ...
        """
        # Use DEFAULT mode to avoid preconditioning memory overhead
        set_mode(self.model, ModuleMode.DEFAULT, tracked_module_names=tracked_module_names, release_memory=True)
        ...
        if loaded_factors:
            try:
                ...
                print("✅ Factors set successfully")
            except Exception as e:
                print(f"⚠️ Failed to set factors: {e}")
        else:
            print("ℹ️ Using identity preconditioning (no factors)")
        ...
        for batch in tqdm(forget_sample_loader, desc="Processing forget sample batches", unit="batch"):
            ...
            with torch.cuda.amp.autocast(enabled=False):  # Disable autocast for gradient computation
            ...
            for j in range(B):
                ...
                if target_positions.numel() == 0:
                    # 메모리 정리
                    del single_out, single_logits, single_logp, single_nll
                    continue
                ...
                for idx_i, tpos in enumerate(target_positions.tolist()):
                    ...
                    # 메모리 정리
                    del grads, grad_theta_jt, Hinv_grad, s_jt
            ...
            # 배치 처리 후 메모리 정리
            del out, logits, logp, nll, valid_labels, gather_indices
            torch.cuda.empty_cache()
...
def parse_args():
    ...
    parser.add_argument("--use_half_precision", action="store_true", default=True)
...
def construct_hf_model(config, checkpoint_dir):    
    ...
    model = AutoModelForCausalLM.from_pretrained(
        checkpoint_dir,
        torch_dtype=torch.bfloat16,  # half precision으로 로딩
...
def get_tokenized_tofu_datasets(config, tokenizer, model_family, max_length):
    # Use TextDatasetQA directly from data_module
    from data_module import TextDatasetQA

    # Create dataset using the split specified in config.forget_split
    dataset_orig = TextDatasetQA(
        data_path=config.data_path,
        tokenizer=tokenizer,
        model_family=model_family,
        max_length=max_length,
        split=config.forget_split,
        question_key='question'
    )

    # Return the same dataset for both (retain dataset will be created separately)
    return dataset_orig
...
def main():
    ...
    config_path = "/root/Unlearn-Simple/TOFU/config/model_config.yaml"
    ...
    config = DataConfig()
    forget_dataset_orig = get_tokenized_tofu_datasets(config=config, tokenizer=tokenizer, model_family=args.model_family, max_length=args.max_length)
    retain_config = DataConfig(data_path=args.tofu_data_path, forget_split=args.retain_split, question_key="question")
    retain_dataset_orig = get_tokenized_tofu_datasets(config=retain_config, tokenizer=tokenizer, model_family=args.model_family, max_length=args.max_length)
    ...
    if hasattr(model, 'gradient_checkpointing_enable'):
        ...
        print("✅ Disabled KV cache for memory optimization")

    # Accelerator 사용 (DeepSpeed 대신)
    ...
    import os
    factors_parent_dir = args.factors_path if args.factors_path else "./factors"
    
    analyzer = DifferentialTokenScorer(
        analysis_name=args.factors_name or "",
        model=model,
        task=task,
