...
import json
import os
from local_utils.utils import get_model_identifiers_from_yaml, add_dataset_index
...
def convert_raw_data_to_model_format(tokenizer, max_length,  question, answer, model_configs):
    ...
    for i in range(min(num_question_tokens, len(label))): label[i] = -100
...
class TNPOForgetDatasetQA(Dataset):
    def __init__(self, data_path, tokenizer, model_family,  max_length=512, split = "forget10", loss_type="idk"):
        ...
        self.forget_mask = None  # 마스크 초기화
        # split 설정은 __init__에서 항상 수행
    ...
    def __getitem__(self, idx):
        ...
        for data_type in [self.split1, self.split2]:
            ...
            # forget 데이터에만 마스크 추가 (원본 idx 사용)
            if data_type == "forget" and self.forget_mask is not None and idx < len(self.forget_mask):
                mask = self.forget_mask[idx]  # 원본 idx 사용
                
                # 마스크 길이를 토큰 길이에 맞게 조정
                if len(mask) < len(converted_data[0]):  # input_ids 길이
                    # 마스크가 짧으면 False로 패딩
                ...
                # 마스크가 없거나 retain 데이터인 경우 빈 마스크 추가
...
class TextForgetDatasetQA(Dataset):
    ...
    def set_forget_mask(self, forget_mask):
        """Dummy method for compatibility - most loss types don't need masks"""
        print(f"TextForgetDatasetQA: forget mask not used for {self.loss_type} loss type")
        pass
...
def dataset_to_json(ds: datasets.Dataset, path: str):
...
class TextForgetDatasetDPOQA(Dataset):
    ...
    def set_forget_mask(self, forget_mask):
        """Dummy method for compatibility - DPO doesn't use masks"""
        print("TextForgetDatasetDPOQA: forget mask not used for DPO loss type")
        pass
